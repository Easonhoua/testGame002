...
function onclickBtn() {
    playBtnAudioFunc()
    //上报注册
    const phoneValue = currentUnit == "R$"?dataRef.value.username:getRawDigits(dataRef.value.username)
    window.jsBridge?.postMessage("registerClick", JSON.stringify({cid: window.ch,phone: phoneValue}))
...
<template>
    ...
    <div v-if="currentTemplate.value =='template_three'">
        <ui-input  v-model="dataRef.username" :error="error_phone" :placeholder="t('modelPage.AccountPhonenumber')"  :class="error_phone?'':'bg-default-bg rounded-lg'"
            @input="handleInput">
            <template #icon>
                <img :src="LoginImg.icon_flag_br" class="w-8 h-6 object-contain block">
            </template>
            <template #left>
                <span class="pr-1 text-sm">{{ currentUnit=="R$"?'+55':'+63' }}</span>
            </template>
            <template #error style="color:black !important;">{{ t('modelPage.enternumber') }}</template>
        </ui-input>
        <p class="mb-3"></p>
        <ui-input v-model="dataRef.password" type="password" :error="error_password" :placeholder="t('userCenter.Password')"  :class="error_password?'':'bg-default-bg rounded-lg'"
            @input="limitPwInput">
            <template #icon>
                <div class="w-8 flex justify-center">
                    <img :src="LoginImg.icon_password" class="w-5 h-5 object-contain block" />
            ...
            <template #error style="color:black !important;">{{ t('modelPage.errortip3') }}</template>
        </ui-input>
        <p class="mb-3"></p>
        <ui-input v-model="dataRef.password_repetition" type="password" :error="error_password_repetition" placeholder="Confirmar senha"  :class="error_password_repetition?'':'bg-default-bg rounded-lg'">
            <template #icon>
        ...
        <div class="flex items-start mt-2 px-1" v-if="isShowTerms">
            ...
            <!-- 移除 for="terms" 取消自动关联 -->
            <div class="ml-2 text-[0.75rem] text-white/80">
                <span class="cursor-default">{{ t('modelPage.agree18') }}</span>
                <span 
                    @click="openTermsModal"
                    class="text-[#ffea00] cursor-pointer hover:underline"
                >
        ...
        <div v-if="showInfoModal" class="fixed inset-0 bg-black/50 flex flex-col items-center justify-center z-50">
            <pu-card theme="1" class="relative w-[19rem] max-h-[80vh] rounded-2xl overflow-hidden m3-theme-bg1">
                <!-- 标题和关闭按钮 -->
                <div class="relative flex items-center justify-between pt-5 mb-4">
                    <h2 class="absolute top-4 left-1/2 -translate-x-1/2 text-l font-bold text-themetext1"> {{ t('modelPage.UserAgreement') }}</h2>
                    <button 
                        @click="closeTermsModal()" 
                        class="absolute top-3 right-3 w-5 h-5 flex items-center justify-center bg-white/10 rounded-full hover:bg-white/20 transition-colors"
                    >
                ...
                <div class="relative z-10 px-3 py-4">
                    <div class="modal-scroll max-h-[50vh] overflow-y-auto pr-2 rounded-xl p-2 text-white">
                        <article class="ctx-article p-mb text-sm">
                            <p>{{  t('modelPage.modelPageDetail5')  }}</p>
                            <p>{{  t('modelPage.modelPageDetail6')  }}</p>
                            <p>{{  t('modelPage.modelPageDetail7')  }}</p>
                            <p>{{  t('modelPage.modelPageDetail8')  }}</p>
                            <p>{{  t('modelPage.modelPageDetail9')  }}</p>
                        </article>
                    </div>
                </div>
                ...
                <div class="w-full flex justify-center px-6 pb-6 pt-2">
                    <button @click="closeTermsModal()" :class="[
                        ...
                        <span>{{ t("modelPage.accepted") }}</span>
                    </button>
                </div>
            </pu-card>
        </div>
    ...
    <div v-if="currentTemplate.value =='template_five'" class="max-h-[22rem] overflow-y-auto rounded-lg">
        <ui-input v-model="dataRef.username" :error="error_phone" placeholder="Número de telefone"  :class="error_phone?'':'bg-logininputcolor1 border border-logininputborder rounded-lg'"
            @input="limitPhoneInput">
            <template #icon>
        ...
        <p class="mb-3"></p>
        <ui-input v-model="dataRef.password" type="password" :error="error_password" placeholder="Senha"  :class="error_password?'':'bg-logininputcolor1 border border-logininputborder rounded-lg'"
            @input="limitPwInput">
            <template #icon>
                <div class="w-8 flex justify-center">
                    <img :src="LoginImg.icon_password" class="w-5 h-5 object-contain block" />
        ...
        <p class="mb-3"></p>
        <ui-input v-model="dataRef.password_repetition" type="password" :error="error_password_repetition" placeholder="Confirmar senha"  :class="error_password_repetition?'':'bg-logininputcolor1 border border-logininputborder rounded-lg'">
            <template #icon>
                <div class="w-8 flex justify-center">
                    <img :src="LoginImg.icon_password" class="w-5 h-5 object-contain block" />
                </div>
        ...
        <div class="w-full pt-[2.5rem] flex justify-center">
            ...
            <button @click="onclickBtn()" :class="auto_click?'':' pointer-events-none'" class="rounded-lg bg-gradient-to-b from-themecardlinear1 to-themecardlinear2 w-full h-10 px-4 !text-sm rounded-lg capitalize text-themewhite">
                <span>Registro</span>
            </button>
            <!-- <img src="/imgs/login/img_5.png" class=" h-5 block absolute right-8 top-1/1 -translate-y-1/2" alt=""> -->
        </div>
