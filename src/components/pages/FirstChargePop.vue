...
<template>
    ...
    <section v-if="showFirstChargeRef" class="w-full h-full fixed left-0 top-0 z-[99] flex items-center justify-center">
        ...
        <div class="content flex flex-col items-center relative" v-if="currentTemplate.value =='template_one'||currentTemplate.value =='template_two'">
            ...
            <div class="table mx-auto z-20  border "  :class="currentTemplate.value =='template_one'?' border-searchborder':' border-defaultborder'">
              ...
              v-for="(item,index) in payListRef.config || []" :key="index">
                <span class="text-rgbawhite80"><span class="text-four">{{ currentUnit.value }}</span> {{item.pay_num}}</span>
                <span  v-if="currentTemplate.value =='template_one'"  class="text-four"><span class="!text-themeLight">{{ currentUnit.value }}</span> {{item.send_num}}</span>
        ...
        <div class="content flex flex-col items-center relative" v-if="currentTemplate.value =='template_three'">
            ...
            <div class="table mx-auto z-20  border mt-5 border-defaultborder">
              ...
              v-for="(item,index) in payListRef.config || []" :key="index">
                ...
                <span class="text-themetext0"><span class="text-themetext1">{{ currentUnit.value }}</span> {{item.send_num}}</span>
              </div>
            </div>
            ...
            <ol class="list-decimal pl-5 text-rgbawhite50" style="font-size: 0.78rem;">
                <li>{{ t("modelPage.modelPageDetail1")  }}</li>
                <li>{{ t("modelPage.modelPageDetail2")  }}</li>
                <li>{{ t("modelPage.modelPageDetail3")  }}</li>
                <li>{{ t("modelPage.modelPageDetail4")  }}</li>
              </ol>
            </div>
        </div>
        </div>
        ...
        <div class="content flex flex-col items-center relative " v-if="currentTemplate.value =='template_five'">
            <button @click="close()" class="flex items-center justify-center font-blod text-3xl" style="width: 3.5rem; height: 3.5rem; z-index: 10;position: absolute; right: -1rem;top: -4rem;">
                ×
            </button>
            <img :src=CommonBgImg.bg_fristchange  class="w-[20rem]  mb-0 z-10 " />
            <div class="overflow-y-auto flex flex-col items-center" :class="isIphoneSE()? 'max-h-[17rem]' : 'max-h-[22rem]'">
              ...
              <div class="flex justify-between items-center  textline even:bg-tablebg2" v-for="(item,index) in payListRef.config || []" :key="index">
                <span class="text-white">{{ currentUnit.value }} <span class="text-themetext0">{{item.pay_num}}</span></span>
                <span class="text-white">{{ currentUnit.value }}<span class="text-themetext0"> {{item.send_num}}</span></span>
              </div>
            </div>
            ...
            <div class="rules w-[19rem]  mt-5">
                ...
                <article class="list-decimal pl-5 text-[0.78rem] text-themewhite" v-html="payListRef.firstpay_content" v-if="payListRef.firstpay_content"></article>
