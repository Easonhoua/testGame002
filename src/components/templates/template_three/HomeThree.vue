        ...
        <pu-page down hideBack height="h-[3.25rem]">
            <template #left>
                <div class="pl-2 flex items-center" >
                    <button @click="openAisde()" class="w-[2.3125rem] h-[2.3125rem] mr-1 bg-transparent rounded-2xl relative cursor-pointer shrink-0 flex items-center justify-center">
                        ...
                        <div v-if="navActivityCountRef>0" class="w-3 h-3 text-[0.5625rem] !leading-none bg-two  text-three  rounded-full absolute -top-0.5 -right-0.5 flex items-center justify-center">
                            <b>{{ navActivityCountRef }}</b>
                        </div>
                    </button>
            ...
            <template #right>
                <div class="pr-2 flex items-center">
                    ...
                    <a href="javascript:;" @click="openService()" class="w-9 h-9 ml-2 rounded-lg m3-theme-btn2 flex items-center justify-center">
            ...
            <!-- banner -->
            <pu-card v-if="bannerListRef&&bannerListRef[1]" theme="3" >
            ...
            <!-- 绿色版不规则样式 -->
            <pu-card theme="3">
                <div class="flex flex-col gap-2" v-if="bannerListRef&&bannerListRef[2]&&bannerListRef[2].length>0">
                    <!-- 上方两个图片 -->
                    <div class="flex gap-2">
                        ...
                        <div class="flex-1  rounded-lg overflow-hidden">
                            <img :src="bannerListRef[2][1].img" 
                                @click="onclickBanner(bannerListRef[2][1])" 
                                class="w-full h-full object-cover banner-img"/>
                        </div>
                    </div>
                    
                    <!-- 下方一个大图片 -->
                    <div class=" rounded-lg overflow-hidden mt-[-0.25rem]">
                        <img :src="bannerListRef[2][2].img"  
                            @click="onclickBanner(bannerListRef[2][2])"
                            class="w-full h-full object-cover banner-img"/>
                    </div>
                </div>
            </pu-card>
            ...
            <!-- notice -->
            <div theme="3" class="!px-0 mt-1">
                <dl class="h-8 flex items-center flex-center" 
                    :style="{background: `url(${HomeImg.img_noticebg}) center` }" style="background-size: cover;">
                    <dt class=" h-full   text-themetext1  flex-1 overflow-hidden flex items-center" >
                    ...
                    <dd @click="toMessage()" class="w-10 h-full relative cursor-pointer shrink-0 flex items-center justify-center ">
                        <img :src=HomeImg.icon_email class="w-[1.5rem] h-[1.5rem] mr-3">
                        <div v-if="parseFloat(msgUnReadCountRef)>0" class="h-3 px-1 text-[0.5625rem] !leading-none bg-two  text-three  rounded-full absolute top-[0.1rem] right-[0.5rem] flex items-center justify-center">
            ...
            <!-- game_cover -->
            <pu-card theme="3" class="pt-0.5 !px-2">
                <van-tabs v-model:active="game_cover_index" shrink 
                    ...
                    <img :src=HomeImg.img_gameTypeBg alt="" class="w-full h-[3.28rem] z-[-1] mt-[-2.5rem]">
                        <van-tab v-for="item,index in gameListRef" :key="index">
                            <template #title>
                                <a @click="openGameAll(item.id)" href="javascript:;" class="h-[3rem] px-1.5">
                                    <div class="game-icon-container ml-2 mr-3 rounded-md flex flex-col items-center justify-center"
                                    >
                                        <img :src="item.cover" class="w-8 h-8 mt-[0.55rem]">
                                        <p class="text-center opacity-65 mt-[0.2rem] text-[0.8rem]" style="max-width: 4rem;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">{{ item.title }}</p>
                                    </div>
                                </a>
                            </template>
                        </van-tab>
            ...
            <pu-card theme="3" class="!px-0">
                <dl v-for="item,index in gameListRef" :key="index" class="mt-10">
                    ...
                    <template v-if="item.title !== 'Favorito'&&item.games.length>0"  >
                        <div class="gameTitle flex flex-col -mt-8"   v-show="item?item.games.length>=0:false" >
                            <div class="w-full flex items-start justify-between">
                                <div class="max-w-[50%] title-badge"     :style="{
                                    background: `url(${HomeImg.img_gameTitleBg}) no-repeat left center / 100% 100%`
                                    }">
                                    <img :src="item.cover" class="w-auto max-w-[60%] h-6 object-contain ml-2">
                                    <b class="ml-1 text-themetext1" style="font-size: 0.88rem;">{{ item.title }}</b>
                                </div>

                                <div class="text-rgbawhite50 flex items-center mt-2 mr-2"  v-show="item?item.games.length>=0:false">
                                <button @click="gamePrevPage(index)" class="w-6 h-6  rounded-md flex items-center justify-center m3-theme-btn2" >
                        ...
                        <div v-if="jpLobbyInfo.status=='1'&&index===0" class="px-1 mt-1">
                            <div @click="goToJackpotsRank()" class="w-full h-[5.5rem] rounded-l overflow-hidden flex items-center justify-between px-1" 
                                ...
                                <!-- 左侧彩金 -->
                              <div class="jackpot-amount bg-cover  w-[10rem] h-[4.6rem] flex items-center justify-center" 
                                ...
                                <div class="w-full h-full overflow-hidden">
                                    ...
                                    <li v-for="(n,i) in [...jpLobbyInfo.list, ...jpLobbyInfo.list]" :key="i" class="w-full flex items-center justify-between my-[0.5rem]">
                                      <div class="text-left leading-tight w-[6.5rem]">
                                        <div class="text-themetext1 text-[0.6rem]">{{ n.username }}</div>
                                        <div class="text-themetext2 text-[0.6rem]">{{ n.text }}</div>
                                      </div>
            ...
            <pwa-guide v-if="home_mode != 3"></pwa-guide>
            <template >
                <!-- 首充活动弹窗 -->
            ...
            <div class="w-full h-auto mb-[3rem] mt-0">
                <div class="w-full h-auto flex flex-col justify-center items-center" 
                    :style="{background: `url(${HomeImg.bg_info}) no-repeat`}" style="background-size: cover;">
                    <img :src=gameLogo class="w-[6rem]  m-auto mt-1 mb-1 max-w-[6.5rem]">
                    <div class="w-full  flex justify-center items-center gap-3 mt-[15rem] mb-[1rem]">
...
.bottomimg{
    width: 100%;
    height: auto;
    background: url('/imgs/home/bg_db_info.png') no-repeat;
...
.gameTitle{
    ...
    /* background-image: url('/imgs/home/home_bg_youxi.png'); */
    /* background-size: 100% 100%; */
    border-radius: 15px; /* 左上角圆角 */
    width: 100%;
    height: auto;
    overflow: hidden;
}
