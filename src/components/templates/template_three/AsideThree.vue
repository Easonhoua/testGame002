                    ...
                    <div class="w-wrap max-w-full h-full mx-auto bg-blur relative">
                        ...
                        <aside class="w-[70%] max-w-[400px] h-full  text-themewhite absolute left-0 top-0 flex flex-col bg-center bg-cover " 
                            ...
                            <section class="w-full px-2 flex-1 overflow-y-auto">
                                <div class="w-full px-3 pb-1 pt-[0.1rem] text-rgbawhite80 h-[8.5rem]" :style="{
                                    ...
                                    <div v-if="isAuthRef" @click="onToProfile()" class="w-full pt-1 flex -mt-1">
                                        <van-image :src="memberLocal.head_portrait||appIcon" width="2.75rem" height="2.75rem" radius="0.5rem" class="mt-1 mr-2  shrink-0" />
                                        <div class="flex-1 overflow-hidden">
                                            <h3 class="mt-1">
                                                <span class="text-sm text-themetext1 align-middle ">{{ memberLocal.nickname }}</span>
                                            ...
                                            <div @click.stop.prevent="copyInviteCode(memberLocal.id)" class="text-xs rounded-lg " style="min-width: 1rem;max-width: 10rem;display: inline-block;" >
                                               <div class="flex items-center">
                                                <span class="align-middle text-themetext2">ID: {{ memberLocal.id }}</span>
                                                <img :src=AsideImg.icon_copy class="w-3 h-3 ml-6 mb-1"/>
                                               </div>
                                ...
                                <!-- <div class="activityBox">
                                    <div class="proBox">
                                        <span class="title text-themefont text-sm">Promoções</span>
                                                ...
                                                <a @click="onclickNav(item.activity_type)" href="javascript:;" class="w-1/2 pt-1">
                                                    <div class="mx-0.5 px-1 rounded-lg flex items-center relative" :style="{ backgroundColor: 'var(--color-tablergba40)' }">
                                                        <div class=" relative mr-1 shrink-0">
                                                            ...
                                                            <div v-if="item.activity_type=='sign_in'&&hashSign" class="w-3 h-3 text-[0.725rem] !leading-none bg-two border-[0.5px] border-white rounded-full absolute top-0.5 -right-0.5 flex items-center justify-center">
                                                                <b>1</b>
                                                            </div>
                                                            <div v-if="item.activity_type=='red_packet'&&redPakageStateRef.status==1" class="w-3 h-3 text-[0.725rem] !leading-none bg-two border-[0.5px] border-white rounded-full absolute top-0.5 -right-0.5 flex items-center justify-center">
                                                                <b>1</b>
                                                            </div>
                                                        </div>
                                                        <div class="flex-1 overflow-hidden">
                                                            <p v-if="item.activity_type=='red_packet'" class="text-left text-[0.8rem] text-themetext1 truncate">
                                                                <span v-if="redPakageStateRef.status>=1" >{{ item.title }}</span>
                                                                <van-count-down v-else :time="redPakageStateRef.time" @finish="redPakageEndFunc()" format="HH:mm:ss" class="!text-themetext1 !text-[0.7rem] " />
                                                            </p>
                                                            <p v-else class="text-left text-[0.8rem] text-themetext1 truncate">{{ item.title }}</p>
                                                        </div>
                                                    </div>
                                                </a>
                                            </template>
