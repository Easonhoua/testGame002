...
<template>
    <pu-page hideHeader>
        ...
        <pu-card v-if="memberRef&&memberRef.id" theme="3" class="pt-5">
            <dl class="flex">
                <dt class="w-[6.75rem] flex items-center flex-col">
                    <div class="w-full flex justify-center">
                        <van-image :src="memberRef.head_portrait" width="4.25rem" height="4.25rem" radius="1rem" class="border border-themewhite/25">
                            <template #loading>
                    ...
                    <div @click.stop.prevent="copyInviteCode(memberRef.id)" class="text-xs bg-default-bg border-none px-1 py-0.5 rounded-lg text-center flex flex items-center " style="min-width: 1rem;max-width: 10rem;display: inline-block;" >
                        <div class="flex items-center">
                        <span class="align-middle  text-one text-[0.64rem]">ID: {{ memberRef.id }}</span>
                ...
                <dd class="pl-5 flex-1 overflow-hidden">
                    ...
                    <ol class="w-full !pt-1 text-xs flex">
                        <li class="w-1/2 pr-1">
                            <div @click="toRecharge()" class="w-full h-8 px-1 unified-button m2-btn-ten  rounded-full flex items-center justify-center">
                                <img :src=CommonImg.img_recharge class="w-5 h-5 mr-1 shrink-0" />
                                <span>Depósito</span>
                            </div>
                        </li>
                        <li class="w-1/2 pl-1">
                            <div @click="toWithdrawal()" class="w-full h-8 px-1 unified-button bg-loginlinear-3 border border-defaultborder rounded-full flex items-center justify-center">
        ...
        <pu-card theme="3" class="mt-3">
            ...
            <div class="w-[11.25rem] h-0 relative z-10">
                <div class="w-full absolute left-4 bottom-0 flex items-center">
                    ...
                    <p class="pb-2.5 text-four">
                        <span class="text-xl">VIP&nbsp;</span>
                        <span class="text-3xl">{{ memberRef&&memberRef.current_level }}</span>
                    </p>
                </div>
            </div>
            <div class="w-full px-2 py-4 -mt-4  bg-gradient-to-r from-minelinear1 to-minelinear2 rounded-lg relative">
                <dl class="w-full block">
                    <dt class="w-full">
                        <span class="text-xs text-nine">Volume acumulado de apostas:</span>
                    ...
                    <dd class="w-full h-2 bg-rgbablack50 rounded-full overflow-hidden flex">
                        <em :style="`width: ${amount_rate}%`" class="h-full bg-four rounded-full flex justify-end overflow-hidden">
                            <i class="w-2 h-2 bg-themewhite rounded-full"></i>
                        </em>
                    </dd>
                    <dd class="w-full text-xs flex items-center justify-between">
                        <span class="text-nine">0</span>
                        <span class="text-nine">{{next_level.bet_amount}}(VIP {{next_level.vip}})</span>
                    </dd>
                </dl>
                <dl class="w-full p-2 mt-2 bg-rgbablack15 rounded-lg block ">
                    <dt class="w-full">
                        <span class="text-xs text-nine">Volume de apostas deste mês:</span>
                        <span class="text-base opacity-85 float-right">{{ fn(memberRef&&memberRef.account&&memberRef.account.consume_money) }}</span>
                        <i class="clear-both block"></i>
                    </dt>
                    <dd class="w-full h-2 bg-rgbablack50 rounded-full overflow-hidden flex">
                        <em style="width: 100%" class="h-full bg-four rounded-full flex justify-end overflow-hidden">
                            <i class="w-2 h-2 bg-themewhite rounded-full"></i>
                        </em>
                    </dd>
                    <dd class="w-full text-xs flex items-center justify-between">
                        <span class="text-nine">0</span>
                        <span class="text-nine">Manter a divisão: 0</span>
                    </dd>
                </dl>
            </div>
        </pu-card>
        <pu-card theme="0" class="mt-3 p-2" >
            ...
            <!-- 从今天起直接佣金 -->
            <div class="w-full py-2 px-3 mb-2 bg-rgbablack50  rounded-lg flex items-center">
                <p class="text-xs opacity-65 flex-1 text-theme overflow-hidden">
            ...
            <p class="text-xs  text-center">Compartilhamento Rápido</p>
            <div class="w-full px-2 bg-rgbablack50 rounded-lg">
                <share-list :showName="true"></share-list>
            </div>
        </pu-card>
        <pu-card theme="0" class="text-sm mt-2">
