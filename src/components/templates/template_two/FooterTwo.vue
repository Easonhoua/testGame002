                ...
                <template v-for="item,index in menu_list" :key="index">
                    ...
                    :style="(item.path === current_path && item.name !== 'Comissão') ? {
                        ...
                        backgroundSize: '100% 100%'
                            ...
                            <img 
                                :src="item.icon" 
                                :class="item.name!=='Comissão'?'w-[2.5rem] h-[2.5rem]':'min-w-[6rem] min-h-[6rem] w-auto h-auto relative -top-[1rem] left-[0.15rem]'"
                            ...
                            <img 
                                ...
                                v-show="item.path === current_path"
                            >
                            <span class="text-[0.7rem] -mt-2 " :class="item.path !== current_path? 'text-theme':'text-three'" v-show="item.name!=='Comissão'">{{ item.name }}</span>
                            <div 
