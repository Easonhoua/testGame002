...
<template>
  <pu-card theme="3" class="relative">
    <section class="w-full p-px mt-4 mb-6">
      <div class="w-full relative  h-[2.6rem] bg-gradient-to-b from-themecardlinear1 to-themecardlinear2 rounded-full  flex items-center justify-center px-2 text-themetext1">
          <img :src="MemberDayImg.img_time" class="w-7 h-8 inline-block mr-2 absolute left-2">
          <span class="text-textcolorsix text-sm">{{ t('activityCenter.EventEndTime') }} <span class="font-bold text-four">{{ endTime }}</span></span>
      </div>
    </section>

    <section class="w-full p-px my-4 h-[5.2rem] bg-gradient-to-b from-card1 to-card2 rounded-lg border border-themetext2 flex flex-col px-2">
      ...
      <div class="w-full h-[2.5rem] bg-rgbablack30 rounded-lg mt-2 flex items-center justify-center">
    ...
    <section class="w-full h-[2rem] p-px mb-8 mt-14 ">
      <div class="w-full flex h-[2.4rem] rounded-full items-center  bg-gradient-to-b from-downloadlinear1 to-downloadlinear2 justify-center text-themewhite  font-bold cursor-pointer"
