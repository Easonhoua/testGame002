...
<template>
  <pu-card theme="3" class="relative">
    <section class="w-full p-px mt-4 mb-6">
      <div class="w-full relative  h-[2.6rem] bg-gradient-to-r from-themecardlinear2 to-themecardlinear1 rounded-full  flex items-center justify-center px-2 text-themetext1">
          <img :src="MemberDayImg.img_time" class="w-7 h-8 inline-block mr-2 absolute left-2">
          <span class="text-one text-sm">{{ t('activityCenter.EventEndTime') }} <span class="font-bold text-four">{{ endTime }}</span></span>
      </div>
    </section>

    <section class="w-full p-px my-4 h-[5.2rem] bg-gradient-to-r from-linear-18 to-linear-19 rounded-lg  flex flex-col px-2">
      ...
      <div class="w-full h-[2.5rem] bg-rgbawhite30 rounded-lg mt-2 flex items-center justify-center">
    ...
    <section class="w-full p-px my-4 h-[5.2rem] bg-gradient-to-r from-linear-18 to-linear-19 rounded-lg  flex flex-col px-2">
      ...
      <div class="w-full h-[2.5rem] bg-rgbawhite30 rounded-lg mt-2 flex items-center justify-center">
          <span class="text-themewhite text-[1.25rem] font-bold mr-10">
            {{ currentUnit.value }}
          </span>
          <span class="text-four text-[1.25rem] font-bold">{{  fn(reward) }}</span>
      </div>
    </section>

    <section class="w-full h-[2rem] p-px mb-8 mt-14 ">
      <div class="w-full flex h-[2.4rem] rounded-full bg-gradient-to-r from-gold-500 to-gold-100 items-center justify-center text-themewhite  font-bold cursor-pointer"
        :class="receive_statusRef != 1 ? 'cursor-not-allowed opacity-50' : ''"
  ...
  <pu-card theme="3" class="mb-3 pt-5 text-box">
    <div class="w-full mb-4 flex items-center justify-center">
      <h5 class="ma-w-[60%] px-2 text-sm text-center text-themewhite font-bold">
        <span>{{ t("ActivityDescription")  }}</span>
      </h5>
    </div>
    <article class="ctx-article p-mb text-sm text-themewhite"  v-html="contentRef">
    </article>
  </pu-card>
</template>
